...
REWARD_NEW_PIXEL = 5.0  # Adjusted for percentage
...
class ExplorationEnv:
    def __init__(self, number_of_bots=5):
        ...
        self.steps = 0
        self.previous_ratio = None  # Will be set after the first step
        self.ratio = None  # Current ratio
        self.polygon_condition = False
    ...
    def smooth_explored_map(self):
        # Create a mask where unexplored areas are 1, others are 0
        unexplored_mask = (self.explored_map == -1).astype(np.uint8)
        
        # Apply morphological opening to remove small isolated unexplored regions
        kernel = np.ones((5, 5), np.uint8)
        smoothed_unexplored_mask = cv2.morphologyEx(unexplored_mask, cv2.MORPH_OPEN, kernel)
        
        # Create a copy of the explored map for smoothing calculations
        smoothed_map = self.explored_map.copy()
        
        # Update the smoothed_map: Where the smoothed unexplored mask is 0 and the original unexplored mask is 1, set to 0
        removed_unexplored = (smoothed_unexplored_mask == 0) & (unexplored_mask == 1)
        smoothed_map[removed_unexplored] = 0
    ...
    def step(self, target_positions):
        ...
        # Store the count of explored free spaces before updating the map
        ...
        # After updating the map, calculate the new count and the difference
        ...
        # Distribute the reward equally among the bots that contributed to new exploration
        if new_explored > 0:
            for idx in range(self.number_of_bots):
                rewards[idx] += (REWARD_NEW_PIXEL * new_explored) / self.number_of_bots

        # Use smoothed map for boundary calculations
        smoothed_map = self.smooth_explored_map()

        # Penalize bots that are within 20 pixels of the 0-100 boundary
        free_space_mask = (smoothed_map == 0).astype(np.uint8)
        obstacle_mask = (smoothed_map == 100).astype(np.uint8)

        # Dilate the free space mask to find adjacent obstacles
        struct_element = np.ones((5, 5), dtype=np.uint8)
        free_space_dilated = cv2.dilate(free_space_mask, struct_element, iterations=1)

        # Find boundary between free space and obstacles
        boundary_free_obstacle = free_space_dilated & obstacle_mask

        # Create the boundary mask for the 0-100 boundary
        boundary_mask_obstacle = boundary_free_obstacle

        # Invert the boundary mask to prepare for distance transform
        boundary_inverted_obstacle = 1 - boundary_mask_obstacle

        # Compute distance transform to the 0-100 boundary
        distance_to_obstacle_boundary = distance_transform_edt(boundary_inverted_obstacle)

        # For each bot, check if it's within 20 pixels of the 0-100 boundary
        for idx, bot_pos in enumerate(self.bot_positions):
            x, y = bot_pos
            dist_to_obstacle_boundary = distance_to_obstacle_boundary[x, y]
            if dist_to_obstacle_boundary <= 20:
                rewards[idx] += -10000  # Penalty for being within 20 pixels of the 0-100 boundary

        # Reward bots that are within 20 pixels of the -1-0 boundary
        unexplored_mask = (smoothed_map == -1).astype(np.uint8)

        # Dilate the free space mask to find adjacent unexplored areas
        free_space_dilated = cv2.dilate(free_space_mask, struct_element, iterations=1)

        # Find boundary between free space and unexplored areas
        boundary_free_unexplored = free_space_dilated & unexplored_mask

        # Create the boundary mask for the -1-0 boundary
        boundary_mask_unexplored = boundary_free_unexplored

        # Invert the boundary mask to prepare for distance transform
        boundary_inverted_unexplored = 1 - boundary_mask_unexplored

        # Compute distance transform to the -1-0 boundary
        distance_to_unexplored_boundary = distance_transform_edt(boundary_inverted_unexplored)

        # For each bot, check if it's within 20 pixels of the -1-0 boundary
        for idx, bot_pos in enumerate(self.bot_positions):
            x, y = bot_pos
            dist_to_unexplored_boundary = distance_to_unexplored_boundary[x, y]
            if dist_to_unexplored_boundary <= 20:
                print(f"Bot {idx} is within 20 pixels of the -1-0 boundary.")
                rewards[idx] += 10000  # Reward for being within 20 pixels of the -1-0 boundary
    ...
    def is_done(self):
        ...
        if max_component_area >= 0.3 * self.max_area:
            self.polygon_condition = True
        else:
            self.polygon_condition = False

        # Condition 2: Ratio between boundaries is greater than 100
        obstacle_mask = (smoothed_map == 100)
        ...
        # Compute the lengths of the boundaries
        ...
        if length_free_unexplored_boundary == 0:
            ...
            self.ratio = length_free_obstacle_boundary / length_free_unexplored_boundary
        ...
        print(f"Step {self.steps}: Polygon Condition = {self.polygon_condition}, Ratio = {self.ratio}")

        # Check both conditions
        if (self.polygon_condition and ratio_condition) or self.steps >= MAX_TIMESTEPS:
            return True
        else:
            return False


# Modified Actor-Critic Model with stochastic policy
class ActorCritic(nn.Module):
    def __init__(self, num_inputs, num_bots):
        ...
        self.conv1 = nn.Conv2d(num_inputs, 16, kernel_size=9, stride=9)
        self.conv2 = nn.Conv2d(16, 32, kernel_size=5, stride=5)
        self.fc = nn.Linear(32 * 20 * 20, 256)

        # Output layers for mean and log_std
        self.policy_mean = nn.Linear(256, num_bots * 2)
        self.policy_log_std = nn.Parameter(torch.zeros(1, num_bots * 2))
    ...
    def forward(self, x):
        ...
        mean = torch.sigmoid(self.policy_mean(x))  # Outputs between 0 and 1
...
def main():
    ...
    for episode in range(NUM_EPISODES):
        ...
        while not done:
            ...
            state_tensor = torch.FloatTensor(state_channels).unsqueeze(0)  # Shape: [1, num_channels, height, width]

            # Get action from model
            mean, std, value = model(state_tensor)  # mean and std shape: [1, num_bots * 2]
            mean = mean.view(num_bots, 2)
            std = std.view(num_bots, 2)

            # Sample actions from the normal distribution
            normal_dist = torch.distributions.Normal(mean, std)
            actions = normal_dist.sample()
            actions = torch.clamp(actions, 0.0, 1.0)  # Ensure actions are within [0, 1]

            # Convert output to positions
            target_positions = []
            for idx in range(num_bots):
                # Get sampled (x, y) in normalized [0,1]
                pred_x = actions[idx, 0].item()
                pred_y = actions[idx, 1].item()

                # Scale to map dimensions
                map_x = int(pred_x * env.actual_map.shape[0])
                map_y = int(pred_y * env.actual_map.shape[1])

                # Ensure positions are within map bounds
                map_x = np.clip(map_x, 0, env.actual_map.shape[0] - 1)
                map_y = np.clip(map_y, 0, env.actual_map.shape[1] - 1)

                # If position is not free explored pixel, find nearest free explored pixel
                if env.explored_map[map_x, map_y] != 0:
                    # Find nearest free explored pixel
                    free_positions = np.argwhere(env.explored_map == 0)
                    if len(free_positions) > 0:
                        distances = np.linalg.norm(free_positions - np.array([map_x, map_y]), axis=1)
            ...
            advantage = returns.sum() - value.squeeze()
            ...
            entropy_loss = normal_dist.entropy().sum(dim=1).mean()
            ENTROPY_COEFF = 0.01  # Adjust as needed
        ...
        print(f"Episode {episode+1}/{NUM_EPISODES}, Total Reward: {cumulative_reward}")
