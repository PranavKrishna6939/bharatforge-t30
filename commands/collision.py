...
class MultiRobotCollisionAvoidance(Node):
    ...
    # def publish_new_goal(self, robot_name, goal_position):
    #     """
    #     Publish new goal for the robot.
    #     """
    ...
    #     # Publish goal to each robot's goal publisher
    #     if robot_name not in self.goal_publishers:
    #         self.goal_publishers[robot_name] = self.create_publisher(PoseStamped, f'/{robot_name}/goal', 10)
    #     self.goal_publishers[robot_name].publish(goal_msg)

def main(args=None):
    rclpy.init(args=args)

    # Create a multi-robot collision avoidance node with 2 robots
    collision_avoidance = MultiRobotCollisionAvoidance(num_robots=4)
