...
class ClosestRobotNavigator(Node):
    def __init__(self):
        ...
        self.get_logger().info("Sending compute path goals to both robots...")
        self.send_compute_goal_to_robot1()
        self.send_compute_goal_to_robot2()

    def send_compute_goal_to_robot1(self):
        if not self.robot1_compute_client.wait_for_server(timeout_sec=5.0):
    ...
    def send_compute_goal_to_robot2(self):
        if not self.robot2_compute_client.wait_for_server(timeout_sec=5.0):
    ...
    def handle_robot1_response(self, future):
        ...
        if not goal_handle.accepted:
            self.get_logger().error("Robot1 goal was rejected.")
            self.robot1_distance = float('inf')
            self.check_and_move_closest_robot()
            return

        goal_handle.get_result_async().add_done_callback(self.process_robot1_response)

    def handle_robot2_response(self, future):
        goal_handle = future.result()
        if not goal_handle.accepted:
            self.get_logger().error("Robot2 goal was rejected.")
            self.robot2_distance = float('inf')
            self.check_and_move_closest_robot()
            return

        goal_handle.get_result_async().add_done_callback(self.process_robot2_response)

    def process_robot1_response(self, future):
        result = future.result().result
        if result:
            self.robot1_distance = self.compute_distance(result.path)
            self.get_logger().info(f"Robot1 estimated path distance: {self.robot1_distance}")
        else:
            self.get_logger().error("Failed to compute path for Robot1.")
            self.robot1_distance = float('inf')
        self.check_and_move_closest_robot()

    def process_robot2_response(self, future):
        result = future.result().result
        if result:
            self.robot2_distance = self.compute_distance(result.path)
            self.get_logger().info(f"Robot2 estimated path distance: {self.robot2_distance}")
        else:
            self.get_logger().error("Failed to compute path for Robot2.")
            self.robot2_distance = float('inf')
        self.check_and_move_closest_robot()
    ...
    def check_and_move_closest_robot(self):
        ...
        if self.robot1_distance < self.robot2_distance:
            ...
            self.get_logger().info("Both robots have the same distance. Defaulting to Robot1.")
            self.send_navigation_goal(self.robot1_navigate_client)

    def send_navigation_goal(self, navigate_client):
        if not navigate_client.wait_for_server(timeout_sec=5.0):
            self.get_logger().error('Navigation server not available.')
            return

        goal_msg = NavigateToPose.Goal()
        goal_msg.pose = self.target_pose
        navigate_client.send_goal_async(goal_msg).add_done_callback(self.handle_navigation_result)
    ...
    def handle_navigation_result(self, future):
        try:
            ...
            if status == 4:  # SUCCEEDED
                ...
                self.get_logger().warn("Navigation was canceled. However, check RViz for the robot's movement.")
